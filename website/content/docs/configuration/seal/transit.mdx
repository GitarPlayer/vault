...
- `token` `(string: <required>)`: The Vault token to use. This may also be
  specified by the `VAULT_TOKEN` environment variable. This does not work with Vault 1.8.4 I tested it several time, when I set the VAULT_TOKEN in journalctl -u vault.service it says missing token. If I then manually set it in the vault.hcl and do systemctl restart vault it immediately works and autounseals. 
